...
function MoviePopover({ movieInfo, offsetX, offsetY, onClose }: MoviePopoverProps) {
    ...
    return (
        <div
            ...
            style={{ position: 'fixed', top: `${offsetY - 120}px`, left: `${offsetX - 56}px`, zIndex: 1 }}
            onMouseLeave={() => {
                setZoomIn('zoom-in');
        ...
        >
            <div className={cx('background-player')}>
                <div className={cx('image-video-wrapper')}>
                    {isBannerVisible && <img src={movieInfo.bgImage} alt="img" className={cx('background-image')} />}
