...
import styles from './RankCarousel.module.scss';
import { MovieInformation } from '~/models';
import RankCarouselCard from '~/components/RankCarouselCard';
...
function RankCarousel({ carouselData, title }: RankCarouselProps) {
                    ...
                    {carouselData.map((item, index) => (
                        <Fragment key={item.id}>
                            <div ref={index === 6 ? sevenChildRef : null} className={cx('carousel-item')}>
                                <RankCarouselCard rank={index} movieInfo={item} />
                            </div>
                        </Fragment>
                    ))}
                </div>
...
export default memo(RankCarousel);
